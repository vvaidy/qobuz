...
function collection(client) {
  ...
  function getTracks(userAuthToken, source, artistId, albumId, query, limit, offset) {
    if (typeof userAuthToken === 'undefined') {
      return Promise.reject('getTracks() requires an user auth token. You should use the user.login() method to get one.');
    }

    if (source && !availableSources.includes(source)) {
      return Promise.reject(`getTracks() source argument is invalid. Available sources are: ${availableSources.join(', ')}.`);
    }

    return client.get('collection/getTracks', { source, artist_id: artistId, albumd_id: albumId, query, limit, offset, user_auth_token: userAuthToken });
  }
}
