...
Playlist.prototype.updatePlaylistsPosition = function (userAuthToken, playlistIds) {
  if (!hasValue(userAuthToken)) {
    return Promise.reject('updatePlaylistsPosition() requires an user auth token. You should use the user.login() method to get one.');
  }

  if (!hasValue(playlistIds)) {
    return Promise.reject('updatePlaylistsPosition() requires playlist IDs.');
  }

  if (!Array.isArray(playlistIds)) {
    return Promise.reject('updatePlaylistsPosition() playlist IDs must be an array.');
  }

  return this.client.get('playlist/updatePlaylistsPosition', { playlist_ids: playlistIds, user_auth_token: userAuthToken });
};

Playlist.prototype.updateTracksPosition = function (userAuthToken, playlistId, playlistTrackIds, insertBefore) {
  if (!hasValue(playlistId)) {
    return Promise.reject('updateTracksPosition() requires a playlist ID.');
  }

  if (!hasValue(insertBefore)) {
    return Promise.reject('updateTracksPosition() requires an insert before value.');
  }

  return this.client.get('playlist/updateTracksPosition', { playlist_id: playlistId, playlist_track_ids: playlistTrackIds, insert_before: insertBefore, user_auth_token: userAuthToken });
};
